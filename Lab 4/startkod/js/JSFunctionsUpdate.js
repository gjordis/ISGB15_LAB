"use strict";
    ...
    //skapar en div att ha checkbox och label i, gjorde detta för att kunna köra space-between mellan checkbox + label med den nuvarande knappen
    ...
    // lägger checkbox och label i den skapade diven
    checkboxDiv.appendChild(knapp);
    ...
    let newGameButton = document.querySelector("#newGame"); // gör en variabel som tilldelas "#newGame" alltså knappen, 

    
    // Lägger till kryssrutan och labeln till DOM-trädet
    form.insertBefore(checkboxDiv, newGameButton); // lägger till checkboxen och label före "#newGame" knappen i DOM
...
let oGameData = {
    ...
    initiateGame: function() {
        ...
        oGameData.timerEnabled = document.querySelector("#my-checkbox") // tilldelar timerEnabled checkboxen för att använda och se om den är checkad eller inte
           
        if (oGameData.timerEnabled.checked) { // Om timerEnabled nu är tilldelad checkboxen är checkad "klickad" ska spelarna köra med timer
            //console.log("startar Timer från initgame");
    ...
    executeMove: function(oEvent) { // oEvent används för att hämnta informationen om vart det klickas i form av "oEvent.target".
        ...
        console.log("timer?: " + oGameData.timerEnabled)
        ...
        if(clicked.tagName == "TD") { //kontrollerar om det som klickas på är en td
            ...
            //console.log("cellindex: " + cellIndex);
                ...
                if(oGameData.gameField[cellIndex] == "" && oGameData.currentPlayer == oGameData.playerOne) { //kollar om cellen är tom och om "currentPlayer" är "playerOne"
                    ...
                    if(oGameData.timerEnabled.checked) { // om timerEnabled är checked "boxen är checkad", starta timer om spelaren trycker
                        oGameData.startTimer();
                    }
                ...
                oGameData.checkForGameOver(); //kallar på funktionen "checkForGameover" för att kolla om någon redan vunnit eller om det är oavgjort
                //console.log("Gameover? :" + oGameData.checkForGameOver() );
                

                
                if(oGameData.checkForGameOver() !== 0){ // så länge checkForGameOver returnerar 0, ingen vinnare när arrayen ej är fylld. finns platser kvar att lägga på
                 if(oGameData.checkForGameOver() === 1) { //Om spelare 1 vunnit
                ...
                }else if(oGameData.checkForGameOver() === 2) { //Om spelare 2 vunnit
                    ...
                    }else if(oGameData.checkForGameOver() === 3) { //Om 3 returneras är det oavgjort
                ...
                spelPlanLyss.removeEventListener("click", oGameData.executeMove); //byter ut "addEventListener" till "removeEventListener"
    ...
    startTimer: function() { // timerfunktion
        ...
        oGameData.timerId = setInterval(function() { // kör setInterval för att ge currentPlayer 5 sekunder på sig att lägga sitt drag
            //console.log("någon timer går");
            //console.trace();
            if(oGameData.currentPlayer == oGameData.playerOne) { //kollar om currentPlayer är "playerOne"
                oGameData.currentPlayer = oGameData.playerTwo; // Om ja, ändrar currentplayer till spelare "playerTwo" 
                oGameData.playerName = oGameData.nickNamePlayerTwo; // Om ja, Ändrar "playerName" till andra spelarens namn (playerTwo)
                document.querySelector(".jumbotron h1").textContent = "Nästa drag: " + oGameData.playerName; //texten i "jumbotron h1" ändras till "playerName"(visar nästa spelare som skall lägga)
                
                
            }else if(oGameData.currentPlayer == oGameData.playerTwo){ //kollar om cellen är tom och om "currentPlayer" är "playerTwo"
                oGameData.currentPlayer = oGameData.playerOne; // Om ja, ändrar currentplayer till spelare ett
                oGameData.playerName = oGameData.nickNamePlayerOne; // Om ja, Ändrar "playerName" till andra spelarens namn (playerOne)
                document.querySelector(".jumbotron h1").textContent = "Nästa drag: " + oGameData.playerName; //texten i "jumbotron h1" ändras till "playerName"(visar nästa spelare som skall lägga)       
            }
    ...
    stopTimer: function() { // funktion för att återställa timer
        ...
        //console.log("före reset efter clear: " + oGameData.timerId);
        oGameData.timerId = null; // säkerställer att timerID blir null
